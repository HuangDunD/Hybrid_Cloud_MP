...
bool handle_one_request(int conn, int rank, int size) {
    ...
    if (!read_full(conn, vtxdist.data(),
                   vtxdist.size() * sizeof(affinity_uds::idx_t))) return false;
    if (!read_full(conn, xadj.data(),
                   xadj.size() * sizeof(affinity_uds::idx_t))) return false;
    if (!read_full(conn, adjncy.data(),
                   adjncy.size() * sizeof(affinity_uds::idx_t))) return false;
    if (hdr.has_vwgt) {
        vwgt.resize(hdr.nvtx_local);
        if (!read_full(conn, vwgt.data(),
                       vwgt.size() * sizeof(affinity_uds::idx_t))) return false;
    }
    if (hdr.has_vsize) {
        vsize.resize(hdr.nvtx_local);
        if (!read_full(conn, vsize.data(),
                       vsize.size() * sizeof(affinity_uds::idx_t))) return false;
    }
    if (hdr.has_adjwgt) {
        adjwgt.resize(hdr.adjncy_len);
        if (!read_full(conn, adjwgt.data(),
                       adjwgt.size() * sizeof(affinity_uds::idx_t))) return false;
    }
    if (hdr.has_prev_part) {
        prev_part.resize(hdr.nvtx_local);
        if (!read_full(conn, prev_part.data(),
                       prev_part.size() * sizeof(affinity_uds::idx_t))) return false;
    }
