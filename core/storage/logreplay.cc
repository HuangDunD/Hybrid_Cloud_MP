...
void LogReplay::apply_sigle_log(LogRecord* log, int curr_offset) {
    ...
    switch(log->log_type_) {
        ...
        case LogType::UPDATE: {
            ...
            printf("machine id:%d ;batch id:%d ;txid:%d \n",update_log->log_node_id_,update_log->log_batch_id_,update_log->log_tid_);
            //yym_tag debug用
            DataItem restoredItem;
            std::memcpy(&restoredItem, update_log->new_value_.value_, sizeof(DataItem));
            float bal = 0;
            std::memcpy(&bal, restoredItem.value + 4, sizeof(float));
            printf("Extracted balance2: %.2f\n",bal);
            // //LOG(INFO)<<"余额为"<<bal;
            //LOG(INFO) << "savings_accid:" <<update_log->new_value_.key_<<" 余额："<<bal;
            // std::stringstream ss;
            // ss << std::setw(7) << std::setfill('0') <<update_log->new_value_.key_ ;
            // ss << " ";
            // ss << std::setw(10) << std::setfill('0') <<bal;
            // std::string s = ss.str();
        //     if(update_log->table_name_[10]=='s')
        //     {overwriteFixedLine("smallbank_savings_test.txt",(update_log->new_value_.key_)+1,s,19);
        //     LOG(INFO) << "savings_accid:" <<update_log->new_value_.key_<<"变化为："<<bal;
        // }
        //     else if(update_log->table_name_[10]=='c')
        //     {overwriteFixedLine("smallbank_checking_test.txt",(update_log->new_value_.key_)+1,s,19);
        //     LOG(INFO) << "checking_accid:" <<update_log->new_value_.key_<<"变化为："<<bal;
        // }
        //     else LOG(INFO) << "error"<<bal;
