...
int main() {
  ...
  assert(Contains(init_thread_body, "PARALLEL_PAGE_FETCH != 0"));
  assert(Contains(init_thread_body, "thread_pool = new ThreadPool"));
  assert(Contains(init_thread_body, "thread_pool = nullptr"));

  std::cout << "interactive initThread initializes parallel fetch thread pool\n";
  return 0;
}
