...
int main(int argc, char *argv[]) {
    ...
    if (unix_socket_path == nullptr && (server_host == nullptr || server_port <= 0)) {
        fprintf(stderr, "Error: provide a Unix socket (-s) or both Host (-h) and Port (-p).\n");
        std::cout << "Examples:\n"
                  << "  ./WookongDB_client -s /path/to/socket\n"
                  << "  ./WookongDB_client -h 127.0.0.1 -p 9095\n";
        return 1;
    }
