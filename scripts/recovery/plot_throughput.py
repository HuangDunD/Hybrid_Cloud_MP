...
用法:
    python3 scripts/recovery/plot_throughput.py <日志目录> <故障时间(ms)> <故障节点ID> [存活节点ID...]
...
def _plot_curves_on_ax(ax, node_tp_data, fault_time_ms, surviving_ids, node_colors,
    ...
    """
    在给定的 ax 上绘制各节点吞吐量曲线（不绘制汇总曲线）。
    如果提供了 fault_node_data，也绘制故障节点的曲线。
    ...
    for idx, nid in enumerate(surviving_ids):
        ...
        ax.plot(rel_ts, vals, color=color, linewidth=1.5, alpha=0.8,
...
def generate_full_timeline_chart(node_tp_data, fault_time_ms, fault_node_id,
    ...
    for nid in surviving_ids:
        if nid in node_tp_data:
            _, vals = node_tp_data[nid]
            if vals:
                all_max_vals.append(max(vals))
    if fault_node_data is not None:
        _, f_vals = fault_node_data
        if f_vals:
            all_max_vals.append(max(f_vals))
    ylim_max = max(all_max_vals) * 1.35 if all_max_vals else 100
