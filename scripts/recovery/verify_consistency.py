...
使用方法：
    python3 scripts/recovery/verify_consistency.py [storage_dir]
...
def verify_table(data_filepath, index_filepath, table_name, expected_magic, expected_value_size):
    ...
    result = {
        ...
        # 少量 lock 残留（≤5）是节点被强制关闭时的正常行为，不视为错误
        'passed': (lock_errors <= 5 and valid_errors == 0 and magic_errors == 0
                   and size_errors == 0 and missing_records == 0),
        'lock_warning': (lock_errors > 0 and lock_errors <= 5),
    }

    if result['passed']:
        if result.get('lock_warning'):
            log_success(f"  ✅ {table_name} 验证通过（{lock_errors} 条 lock 残留，属于节点关闭时的正常行为）: {verified_count}/{total_records} 条记录")
        else:
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        # 默认路径：项目根目录下的 build/storage_server
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
        storage_dir = os.path.join(project_root, 'build', 'storage_server')
