...
#
# 使用方法：
#   ./scripts/recovery/test_recovery.sh       # 运行故障恢复测试
#   KILL_DELAY=20 ./scripts/recovery/test_recovery.sh
#   ./scripts/recovery/test_recovery.sh stop  # 停止所有节点
# ============================================================================

SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
WORKSPACE=$(cd "${SCRIPT_DIR}/../.." && pwd)
BUILD_DIR="${WORKSPACE}/build"
SESSION_NAME="${RECOVERY_SESSION_NAME:-recovery_test}"
...
COMPUTE_NODES=(0 1 2)
# 等待多少秒后 kill 计算节点
KILL_DELAY="${KILL_DELAY:-35}"
# 故障注入后等待多少秒关闭所有节点
SHUTDOWN_DELAY="${SHUTDOWN_DELAY:-40}"
...
tmux new-window -t ${SESSION_NAME} -n "compute0" \
    "cd ${BUILD_DIR}/compute_server/ && ./compute_server smallbank lazy 5 0.1 0.9 0 2>&1 | tee ${LOG_DIR}/compute0_${TIMESTAMP}.log; echo '--- 计算节点0已退出 ---'; read"
sleep 1

# 4. 启动计算节点 1
log_info "[4/5] 启动计算节点 1..."
tmux new-window -t ${SESSION_NAME} -n "compute1" \
    "cd ${BUILD_DIR}/compute_server/ && ./compute_server smallbank lazy 5 0.1 0.9 1 2>&1 | tee ${LOG_DIR}/compute1_${TIMESTAMP}.log; echo '--- 计算节点1已退出 ---'; read"
sleep 1

# 5. 启动计算节点 2
log_info "[5/5] 启动计算节点 2..."
tmux new-window -t ${SESSION_NAME} -n "compute2" \
    "cd ${BUILD_DIR}/compute_server/ && ./compute_server smallbank lazy 5 0.1 0.9 2 2>&1 | tee ${LOG_DIR}/compute2_${TIMESTAMP}.log; echo '--- 计算节点2已退出 ---'; read"
...
if [ -n "$VICTIM_PANE_PID" ]; then
    ...
    VICTIM_PID=$(pgrep -f "compute_server smallbank lazy 5 0.1 0.9 ${VICTIM_ID}" | head -1)
...
python3 "${SCRIPT_DIR}/analyze_recovery_time.py" "${LOG_DIR}" ${SURVIVING_ARGS} 2>&1 | tee "${RECOVERY_TIME_LOG}"
...
log_info "先等待 2s 让正在执行的事务完成..."
sleep 2
for nid in "${SURVIVING_NODES[@]}"; do
    COMPUTE_PID=$(pgrep -f "compute_server smallbank lazy 5 0.1 0.9 ${nid}" | head -1)
    if [ -n "$COMPUTE_PID" ]; then
        log_info "  关闭计算节点 ${nid} (PID: ${COMPUTE_PID})..."
        kill -INT ${COMPUTE_PID} 2>/dev/null
        sleep 2
        # 如果还没退出，强制 kill
        if kill -0 ${COMPUTE_PID} 2>/dev/null; then
            kill -9 ${COMPUTE_PID} 2>/dev/null
        fi
...
python3 "${SCRIPT_DIR}/verify_consistency.py" "${STORAGE_DATA_DIR}" 2>&1 | tee "${VERIFY_LOG}"
...
python3 "${SCRIPT_DIR}/plot_throughput.py" "${LOG_DIR}" "${FAULT_TIME_MS}" "${VICTIM_ID}" ${TP_SURVIVING_ARGS} 2>&1 | tee "${TP_CHART_LOG}"
