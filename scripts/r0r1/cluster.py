...
def main():
    ...
    def spawn(role, binary, argv, ready_ports):
        ...
        if args.request_driver and role.startswith('compute_'):
            ...
            # R2c: 恢复期 admission 模式（paged=按页准入，**默认**，无需设置；
            #  global=R2 全局屏障对照模式，显式设置才生效）。compute 进程内
            #  getenv 读取，须显式透传
            if os.environ.get('HCM_RECOVERY_ADMISSION'):
                env['HCM_RECOVERY_ADMISSION'] = os.environ['HCM_RECOVERY_ADMISSION']
            # R3 (22.8): 可插拔恢复优先策略（OFF=默认原路径零改动；
            #  B0=接口版公共基线，恒等重排+接口开销单列）。compute 进程内
            #  getenv 读取，须显式透传
            if os.environ.get('HCM_RECOVERY_POLICY'):
                env['HCM_RECOVERY_POLICY'] = os.environ['HCM_RECOVERY_POLICY']
