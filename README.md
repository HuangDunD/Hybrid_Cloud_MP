# WookongDB MP 
<img src="docs/assets/logo.png" alt="WookongDB-MP" width="600" />
...
- `client/`：SQL 命令行客户端。
- `compute_server/`：计算节点；页面与事务策略集中在 `compute_server/strategies/`，执行线程集中在 `compute_server/worker/`。
- `core/`：存储、事务、索引、RPC、协程与 SQL 执行引擎等核心组件。
- `remote_server/`：元信息与远程页表服务。
